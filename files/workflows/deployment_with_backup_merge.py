"""
workflows/deployment_with_backup_merge.py - Deployment WITH project backup via merge (nach PD).

Replaces: BAT_SGB_II_MaEnde_D2D.BAT + CM_SGB_II_MaEnde_nach_PD.scp
          + ProjectMerge.exe + D_SGB_II_MaEnde_D2D.xml  (future, when supported)
...
When to use:
    ...
    Command: python main.py mit-backup-merge --backup-month 202512 [--env deployment.env]
    The --backup-month value is appended to BACKUP_PROJECT_BASE_NAME, e.g.:
        "SGB II MaEnde" + "202512" -> "SGB II MaEnde 202512"

Backup method is "merge" (when implemented).

Steps:
    1. Disconnect all active users from the main project
    2. Create a backup project via merge (NOT YET IMPLEMENTED - placeholder)
...
def run(cfg: AppConfig, backup_month: str) -> bool:
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("  WORKFLOW: mit Backup Merge (with backup via merge / nach PD) - FUTURE FEATURE")
    ...
    with mstr_connection(cfg.mstr) as conn:
        ...
        # Creates a backup of the main project via merge (NOT YET IMPLEMENTED).
        # This is a placeholder for future mstrio support.
        logger.info("\n[Step 2/7] Create backup project via merge")
        if cfg.project.backup_method != "merge":
            logger.warning(f"  Backup method is '{cfg.project.backup_method}', expected 'merge' for this workflow")
        # TODO: Implement merge when mstrio supports it
        raise NotImplementedError("Project merge backup is not yet implemented in mstrio-py. Use 'mit-backup' for duplication instead.")
